 ...
 */
euclid::euclid(const euclid &copy):A(copy.A), B(copy.B), modCalls(copy.modCalls),gcd(copy.gcd),I(copy.I) {};
 ...
 * the data later.
 */
euclid::euclid(const long long int &a, const long long int &b,const long long int &i):A(a),B(b),modCalls(0),gcd(1),I(i) {
 ...
 */
 ...
 long long int euclid::getGCD() const { return gcd; }
 long long int euclid::getModCalls() const { return modCalls; }
 long long int euclid::getA() const { return A; }
 long long int euclid::getB() const { return B; }
 long long int euclid::getI() const { return I; }


std::ostream &operator<<(std::ostream &os, const euclid &g) {
    os << g.getI() << "," << g.getA() << "," << g.getB() << "," << g.getGCD() << "," << g.getModCalls();
    return os;
}


bool euclid::operator>(const euclid &rhs) const { return modCalls > rhs.modCalls; }
...
bool euclid::operator==(const euclid &rhs) const { return modCalls == rhs.modCalls && A == rhs.A && B == rhs.B; }

euclid &euclid::operator=(const euclid &rhs) {
    I = rhs.getI();
    A = rhs.getA();
    B = rhs.getB();
    gcd = rhs.getGCD();
